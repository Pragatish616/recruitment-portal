...
2. **No maximum length on free-text fields** (`Name`, per-department question answers) — validated for presence, not length. Bounded in practice by the existing 2-applications-per-account cap and sign-up rate limiting, so low severity, but a real gap if judged specifically against this project's "storage-cost" angle. Not fixed this round (flagged, not blocking — see §14).
