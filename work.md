...
2. **No maximum length on free-text fields** (`Name`, per-department question answers) — validated for presence, not length. Bounded in practice by the existing 2-applications-per-account cap and sign-up rate limiting, so low severity, but a real gap if judged specifically against this project's "storage-cost" angle. Not fixed this round (flagged, not blocking — see §16).
...
## 15. Password strength requirement on sign-up (2026-09-08)

Weak passwords (e.g. `password`, `12345678`) were previously accepted outright — better-auth's own default only enforces a minimum length of 8 characters, no composition rule. New requirement: **at least 8 characters, with at least one letter, one number, and one special character** — a common baseline on production sites with real auth requirements.

**Enforced server-side, not just in the UI** (the same principle applied everywhere else in this document — client-side validation alone has repeatedly turned out to be bypassable by a direct API call). Implemented via better-auth's own `hooks.before` extensibility point in `lib/auth.js`, matched specifically on `ctx.path === "/sign-up/email"` — this is a real path-based guard, not a heuristic, so every other request (sign-in, session checks, OAuth callbacks) returns immediately untouched. Verified live, not just by reading the code: a weak password (`weak`) posted directly to `/api/auth/sign-up/email` was rejected with a `400` and the custom message *before* any database write was attempted; a strong password (`Str0ng!Pass`) passed the same check and proceeded to the (separate, pre-existing, environment-specific) database step, confirming the rule discriminates correctly rather than blocking everything.

**Existing accounts are completely untouched, by construction, not by a special-case exemption:** the hook only ever fires for the sign-up path — an existing account's stored password hash is never re-checked, never invalidated, and its owner is never prompted to change it. Nobody who could already sign in is affected; this only applies to accounts created from this point forward.

**Client side** (`app/auth/signin/page.jsx`): the same rule is checked instantly on submit for fast feedback (avoiding an unnecessary round trip for an obviously-weak password), and the expected format is shown as a persistent hint below the password field in sign-up mode — not just as an error after a failed attempt — matching the pattern used by production auth flows that state the requirement up front rather than making the user guess and fail first.

## 16. Recommended next improvements (post-deadline, prioritized)
