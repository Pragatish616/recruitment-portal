...
// top of better-auth's own default minPasswordLength: 8. Only gates new
// sign-ups (matched on ctx.path below) - existing accounts' passwords are
// left exactly as they are; nobody is forced to change anything.
...
export const auth = betterAuth({
  ...
  // Runs on every better-auth request (the library's own hook contract has
  // no way to scope a middleware to one path up front), so it immediately
  // no-ops for anything that isn't a fresh sign-up - sign-in for an existing
  // account, session checks, and OAuth callbacks are all unaffected, meaning
  // no existing password is ever re-validated or forced to change.
  hooks: {
    before: createAuthMiddleware(async (ctx) => {
      if (ctx.path !== "/sign-up/email") return;
      const password = ctx.body?.password;
      if (typeof password !== "string" || !PASSWORD_STRENGTH_REGEX.test(password)) {
